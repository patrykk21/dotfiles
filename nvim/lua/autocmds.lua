...
vim.g.closed_buffers = vim.g.closed_buffers or {}
vim.api.nvim_create_autocmd("BufDelete", {
  group = vim.api.nvim_create_augroup("TrackClosedBuffers", { clear = true }),
  callback = function(args)
    local bufname = vim.fn.bufname(args.buf)
    if bufname ~= "" then
      -- Update the table and reassign it back to vim.g
      local closed_buffers = vim.g.closed_buffers
      table.insert(closed_buffers, bufname)
      vim.g.closed_buffers = closed_buffers
    end
  end,
})
