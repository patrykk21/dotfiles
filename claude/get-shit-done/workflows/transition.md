...
```bash
TRANSITION=$(node /Users/vigenerr/.claude/get-shit-done/bin/gsd-tools.cjs phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node /Users/vigenerr/.claude/get-shit-done/bin/gsd-tools.cjs progress bar --raw)
```
...
```bash
node /Users/vigenerr/.claude/get-shit-done/bin/gsd-tools.cjs config-set workflow.auto_advance false
```
