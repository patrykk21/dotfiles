...
```bash
INIT=$(node /Users/vigenerr/.claude/get-shit-done/bin/gsd-tools.cjs init quick "$DESCRIPTION")
```
...
```bash
node /Users/vigenerr/.claude/get-shit-done/bin/gsd-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
